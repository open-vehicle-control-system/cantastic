defmodule Cantastic.ReceivedFrameWatcher do
  ...
  def handle_info(:validate_frequency, state) do
    ...
    cond do
      ...
      is_alive && is_late ->
        {:noreply,
         %{state | on_time_frame_count: 0, missed_frame_count: state.missed_frame_count + 1}}
      ...
      !is_alive && !is_late ->
        {:noreply,
         %{state | on_time_frame_count: state.on_time_frame_count + 1, missed_frame_count: 0}}
  ...
  defp ensure_frequency!(state) do
    if !state.frame_frequency do
      throw(
        "[Yaml configuration error] Watched received frame '#{state.network_name}.#{state.frame_name}' is missing a frequency, please add it in the Yaml configuration."
      )
    end
  end
end
