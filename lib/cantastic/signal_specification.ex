defmodule Cantastic.SignalSpecification do
  ...
  defp validate_keys!(network_name, frame_name, yaml_signal_specification) do
    ...
    if invalid_keys != [] do
      throw(
        "[Yaml configuration error] Signal '#{network_name}.#{frame_name}.#{yaml_signal_specification.name}' is defining invalid keys: #{invalid_keys |> Enum.join(", ")}"
      )
    end
  end
  ...
  def validate_specification!(signal_specification) do
    if is_nil(signal_specification.name) do
      throw(
        "[Yaml configuration error] Signal '#{signal_specification.network_name}.#{signal_specification.frame_name}.#{signal_specification.name}' is missing a 'name'."
      )
    end

    if is_nil(signal_specification.value_start) do
      throw(
        "[Yaml configuration error] Signal '#{signal_specification.network_name}#{signal_specification.frame_name}.#{signal_specification.name}' is missing a 'value_start'."
      )
    end

    if is_nil(signal_specification.value_length) do
      throw(
        "[Yaml configuration error] Signal '#{signal_specification.network_name}#{signal_specification.frame_name}.#{signal_specification.name}' is missing a 'value_length'."
      )
    end

    if !Enum.member?(@valid_kinds, signal_specification.kind) do
      throw(
        "[Yaml configuration error] Signal '#{signal_specification.network_name}#{signal_specification.frame_name}.#{signal_specification.name}' is using an invalid kind: '#{signal_specification.kind}'"
      )
    end

    if !Enum.member?(@valid_signs, signal_specification.sign) do
      throw(
        "[Yaml configuration error] Signal '#{signal_specification.network_name}#{signal_specification.frame_name}.#{signal_specification.name}' is using an invalid sign: '#{signal_specification.sign}'"
      )
    end

    if !Enum.member?(@valid_endiannesses, signal_specification.endianness) do
      throw(
        "[Yaml configuration error] Signal '#{signal_specification.network_name}#{signal_specification.frame_name}.#{signal_specification.name}' is using an invalid endianness: '#{signal_specification.endianness}'"
      )
    end
    ...
    case signal_specification.kind do
      "enum" ->
        if !is_map(signal_specification.mapping) || signal_specification.mapping == %{} do
          throw(
            "[Yaml configuration error] A mapping is missing for the signal '#{signal_specification.network_name}#{signal_specification.frame_name}.#{signal_specification.name}' of kind 'enum'."
          )
        end
      ...
      "static" ->
        if is_nil(signal_specification.value) do
          throw(
            "[Yaml configuration error] A value is missing for the signal '#{signal_specification.network_name}#{signal_specification.frame_name}.#{signal_specification.name}' of kind 'static'."
          )
        end
  ...
  defp compute_mapping(mapping, value_length) do
    mapping
    |> Map.keys()
    |> Enum.reduce(%{}, fn atom_key, computed_mapping ->
      key =
        atom_key
        |> Atom.to_string()
        |> Util.string_to_integer()
        |> Util.integer_to_bin_big(value_length)
  ...
  defp compute_reverse_mapping(mapping, value_length) do
    mapping
    |> Map.keys()
    |> Enum.reduce(%{}, fn atom_value, computed_mapping ->
