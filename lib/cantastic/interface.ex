defmodule Cantastic.Interface do
      ...
      |> Enum.map(fn network ->
        {:ok, socket} =
          initialize_socket(
            network.interface,
            network.bitrate,
            ConfigurationStore.setup_can_interfaces()
          )

        Logger.info(
          "CAN network: '#{network.network_name}' ready on interface: '#{network.interface}'"
        )
  ...
  def configure_receivers_and_watchers(interface_specs) do
    ...
    |> Enum.map(fn %{network_name: network_name, network_config: network_config, socket: socket} ->
      ...
      received_frame_specifications =
        compute_frame_specifications(
          network_config[:received_frames] || [],
          network_name,
          :receive
        )
      ...
      watchers =
        ...
        |> Enum.map(fn {_id, frame_specification} ->
          arguments = %{
            process_name:
              received_frame_watcher_process_name(network_name, frame_specification.name),
    ...
    |> Enum.map(fn %{
      ...
      |> Enum.map(fn yaml_request_specification ->
        {:ok, request_specification} =
          OBD2.RequestSpecification.from_yaml(network_name, interface, yaml_request_specification)
  ...
  def emitter_process_name(network_name, frame_name) do
    "#{process_name_prefix(network_name)}#{frame_name |> Macro.camelize()}Emitter"
    |> String.to_atom()
  end

  def obd2_request_process_name(network_name, request_name) do
    "#{process_name_prefix(network_name)}#{request_name |> Macro.camelize()}OBD2Request"
    |> String.to_atom()
  end

  def received_frame_watcher_process_name(network_name, frame_name) do
    "#{process_name_prefix(network_name)}#{frame_name |> Macro.camelize()}ReceivedFrameWatcher"
    |> String.to_atom()
  end
  ...
  defp compute_frame_specifications(yaml_frame_specifications, network_name, direction) do
    ...
    |> Enum.reduce(%{}, fn yaml_frame_specification, frame_specifications ->
      ...
      if Enum.find(frame_specifications, fn {_id, fs} -> fs.name == frame_specification.name end) do
        throw(
          "[Yaml configuration error] frame name: '#{frame_specification.name}' is duplicated, please ensure that you only use a frame name once per network."
        )
      end
  ...
  defp setup_can_interface(interface, _bitrate, false = _setup_can_interfaces, _retry_number) do
    Logger.debug(
      "Connection to the CAN bus #{interface} skipped following can interface setup config"
    )

    :ok
  end

  defp setup_can_interface(interface, bitrate, setup_can_interfaces, retry_number)
       when binary_part(interface, 0, 4) == "vcan" do
    with {_output, 0} <- System.cmd("ip", ["link", "add", "dev", interface, "type", "vcan"]),
         ...
         false <- output |> String.match?(~r/state DOWN/) do
  ...
  defp setup_can_interface(interface, bitrate, true = setup_can_interfaces, retry_number) do
    with {_output, 0} <-
           System.cmd("ip", ["link", "set", interface, "type", "can", "bitrate", "#{bitrate}"],
             stderr_to_stdout: true
           ),
         {_output, 0} <-
           System.cmd("ip", ["link", "set", interface, "txqueuelen", "1000"],
             stderr_to_stdout: true
           ),
         {_output, 0} <-
           System.cmd("ip", ["link", "set", interface, "up"], stderr_to_stdout: true) do
      Logger.info(
        "Connection to the CAN bus #{interface} with a  bitrate of #{bitrate} bit/seconds initialized"
      )
      ...
      {output, _} ->
        Logger.warning(
          "The connection to the CAN bus interface #{interface} failed with the following reason: '#{output}'Retrying in 0.5 seconds."
        )
