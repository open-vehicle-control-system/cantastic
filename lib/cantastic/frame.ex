defmodule Cantastic.Frame do
  ...
  defp build_raw_data(frame_specification, parameters) do
    raw_data =
      ...
      |> Enum.reduce(<<>>, fn signal_specification, raw_data ->
        ...
        if is_nil(raw_signal),
          do:
            throw(
              "Signal '#{frame_specification.network_name}.#{frame_specification.name}.#{signal_specification.name}' value is missing from emitter data."
            )
  ...
  @doc false
  defp include_checksum_if_required(raw_data, %FrameSpecification{checksum_required: false}, _),
    do: raw_data
  ...
  def interpret(frame, frame_specification) do
    signals =
      frame_specification.signal_specifications
      |> Enum.reduce(%{}, fn signal_specification, acc ->
