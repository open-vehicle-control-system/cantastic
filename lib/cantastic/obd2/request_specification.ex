defmodule Cantastic.OBD2.RequestSpecification do
  ...
  def from_yaml(network_name, can_interface, yaml_obd2_request_specification) do
    ...
    {:ok, parameter_specifications} =
      parameter_specifications(
        network_name,
        yaml_obd2_request_specification.name,
        yaml_parameter_specifications
      )
  ...
  defp validate_keys!(network_name, yaml_obd2_request_specification) do
    ...
    if invalid_keys != [] do
      throw(
        "[Yaml configuration error] OBD2 Request '#{network_name}.#{yaml_obd2_request_specification.name}' is defining invalid key: #{invalid_keys |> Enum.join(", ")}"
      )
    end
  end
    ...
    computed =
      ...
      |> Enum.map(fn yaml_parameter_specification ->
        {:ok, parameter_specification} =
          ParameterSpecification.from_yaml(
            network_name,
            request_name,
            yaml_parameter_specification
          )
  ...
  defp validate_parameter_specifications!(parameter_specifications) do
    parameter_specifications
    |> Enum.each(fn parameter_specification ->
